...
void mcl::initializeParticles(){
    ...
    */
    std::uniform_real_distribution<float> x_pos(mapCenterX - gridMap_use.cols * imageResolution / 4.0,
                mapCenterX + gridMap_use.cols * imageResolution / 4.0);
    std::uniform_real_distribution<float> y_pos(mapCenterY - gridMap_use.rows * imageResolution / 4.0,
                mapCenterY + gridMap_use.rows * imageResolution / 4.0);
    // std::uniform_real_distribution<float> z_pos(mapCenterZ - gridMap_use.?? * imageResolution / 4.0,
    //             mapCenterZ + gridMap_use.?? * imageResolution / 4.0);
...
void mcl::prediction(Eigen::Matrix4f diffPose){
    ...
    double delta_rot1 = atan2(diff_xyzrpy(1), diff_xyzrpy(0)); // 이동 전 로봇이 목표 방향으로 향하기 위해 회전한 각도 ?
    double delta_rot2 = diff_xyzrpy(5) - delta_rot1; // 목표 위치로 이동 후 추가로 회전한 각도 ?
    ...
    for(int i=0; i<particles.size(); ++i){
        ...
        double y = delta_trans * sin(delta_rot2) + gaussian_distribution(gen) * odomCovariance[5];
...
void mcl::showInMap(){
    ...
    for(int i=0; i<numOfParticle; ++i){
        ...
        cv::circle(showMap, cv::Point(poseX_px, poseY_px), 1, cv::Scalar(255,0,0), -1);
    }
